...
export default function VideoScreeningPage() {
  ...
  if (!candidate.videoScreening) {
    return (
      <div className="flex items-center justify-center h-[60vh]">
        <div className="text-center">
          <h2 className="text-xl font-semibold mb-2">No video screening available</h2>
  ...
  const handleLoadedMetadata = () => {
    if (videoRef.current) {
      setDuration(videoRef.current.duration || screening.duration)
    }
  }
      ...
      <div className="grid grid-cols-3 gap-6">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-muted-foreground ml-2">
                    {formatTime(currentTime)} / {formatTime(duration || screening.duration)}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <Button
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <button className="text-xs text-primary mt-2 hover:underline">
                        Jump to response ({formatTime(q.timestamp)})
                      </button>
                    </div>
                  </div>
                </div>
        ...
        <div className="space-y-4">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <p className="text-sm text-muted-foreground">{screening.aiAnalysis}</p>
              <Separator />
              <div className="space-y-3">
                <div>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs text-muted-foreground">Communication</span>
                    <span className="text-xs font-medium">{screening.communicationScore}%</span>
                  </div>
                  <Progress value={screening.communicationScore} className="h-1.5" />
                </div>
                <div>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs text-muted-foreground">Confidence</span>
                    <span className="text-xs font-medium">{screening.confidenceScore}%</span>
                  </div>
                  <Progress value={screening.confidenceScore} className="h-1.5" />
                </div>
                <div>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs text-muted-foreground">Technical Clarity</span>
                    <span className="text-xs font-medium">{screening.technicalScore}%</span>
                  </div>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-3 gap-2">
                ...
                <Button
                  ...
                  className={decision === "hold" ? "bg-amber-600 hover:bg-amber-700" : ""}
                >
              ...
              />
              <Button className="w-full" onClick={handleSubmitDecision} disabled={!decision}>
                Save Decision
              </Button>
              {screening.recruiterDecision && (
                <p className="text-xs text-muted-foreground text-center">
                  Last updated: {format(new Date(screening.recruiterDecision.decidedAt), "MMM d, yyyy 'at' h:mm a")}
                </p>
              )}
            </CardContent>
          </Card>
